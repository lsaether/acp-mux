...
- **AMUX active-turn steer/queue controls.** `amux/steer_active_turn` and `amux/queue_prompt` are now the canonical busy-time control surface. The mux translates accepted controls to compatible sideband prompts without opening a second mux turn, stealing the active driver, or replaying the requester’s private response. Plain concurrent `session/prompt` requests—including text that starts with `/steer ...` or `/queue ...`—remain serialized and receive `-32001`. Fixes [#39](https://github.com/lsaether/acp-mux/issues/39).
