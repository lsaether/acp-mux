//! Opt-in on-disk persistence for the per-room broadcast replay log.
//!
//! Enabled by `--replay-store <DIR>`. One JSONL file per room
//! (`<DIR>/<room_id>.jsonl`); every persisted line is one broadcast-tier
//! frame in the same order it flowed through `RoomInner::broadcast`.
//!
//! Record schema (v=1):
//!
//! ```json
...
//!   `ReplayEntry`. `segment_id` of 0 means the pre-segment sentinel.
...
//!   `ReplayEntry::frame_for_replay()` using the persisted `recorded_at`
//!   and `seq`, preserving the contract that mux-recorded time wins.
//!
