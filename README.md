...
Then connect WebSocket clients to `ws://127.0.0.1:8765/acp?session=<id>&peer_id=<unique>&peer_name=<display>&role=<optional>`. Attach-aware clients can add `&replay=skip` and use `session/attach.result.history` as their single bootstrap history source.
