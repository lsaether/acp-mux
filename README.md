...
- **Turn serialization.** Concurrent ordinary `session/prompt` while a turn is in flight is rejected with JSON-RPC `-32001`; active-turn steering/queueing goes through explicit `amux/steer_active_turn` and `amux/queue_prompt` requests. Those controls are translated to compatible sideband prompts without stealing the active driver or emitting turn bookends. Agent-emitted acknowledgements for those controls (Hermes sends them as `session/update` text, including `/queue` depth) still broadcast/replay to every peer. The last subscriber to issue a substantive request is still surfaced as the "driving subscriber" in `/debug/sessions` and `amux/turn_started` for UI attribution.
...
- **`amux/*` extension namespace.** The mux publishes its own metadata/control plane out-of-band: `amux/session_context`, `amux/peer_joined`, `amux/peer_left`, `amux/turn_started`, `amux/turn_complete`, `amux/turn_cancelled`, `amux/session_busy`, `amux/agent_request_opened`, `amux/agent_request_resolved`, plus subscriber-request controls such as `amux/steer_active_turn`, `amux/queue_prompt`, and `amux/cancel_active_turn`. ACP frames stay clean; clients see two distinguishable channels and demultiplex by method prefix.
...
| `amux/steer_active_turn` | subscriber → proxy | Steer the active turn through the mux control plane; translated to a compatible sideband prompt without opening a mux turn. |
| `amux/queue_prompt` | subscriber → proxy | Queue text for a compatible agent while another turn is active; translated to a compatible sideband prompt without opening a mux turn. |
