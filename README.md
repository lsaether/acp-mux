...
- **Replay log.** Every broadcast-tier frame (`amux/*` + agent notifications) is appended. By default, a late joiner receives chronological history before later live events. With `replay_order=newest_turn_first`, the late joiner first receives `amux/session_snapshot`, then `amux/replay_started`, then the newest turn segment immediately; older turn segments backfill newest-to-oldest in the background, so later live frames may arrive before `amux/replay_complete`. Raw collaborative agent-initiated requests remain historical-lifecycle-only, except that current in-flight turn-scoped `session/request_permission` requests are re-offered as actionable raw requests during attach. Ambiguous non-turn request re-offer semantics are deferred to Phase 3.
...
- Treat `amux/replay_started` / `amux/replay_complete` as newest-first backfill lifecycle markers. Live frames (and attach-time pending permission re-offers) can arrive before `amux/replay_complete`; replayed historical frames carry replay provenance under `_meta.amux`.
- Treat `amux/turn_started` / `amux/turn_complete` as turn bookends; the `peerId` field attributes the turn.
- Treat `amux/agent_request_opened` / `amux/agent_request_resolved` as the non-actionable lifecycle for agent-initiated requests. Only raw `session/request_permission` requests should create a reply affordance; in newest-first mode the mux may re-offer a still-in-flight turn-scoped permission to a late joiner, but replayed `amux/agent_request_opened` is context, not a request to answer.
