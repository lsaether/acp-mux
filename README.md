...
- **Turn serialization.** Concurrent ordinary `session/prompt` while a turn is in flight is rejected with JSON-RPC `-32001`; text-only Hermes `/steer ...` and `/queue ...` prompts are forwarded as sideband requests without stealing the active driver or emitting turn bookends. Agent-emitted acknowledgements for those control prompts (Hermes sends them as `session/update` text, including `/queue` depth) still broadcast/replay to every peer. The last subscriber to issue a substantive request is still surfaced as the "driving subscriber" in `/debug/sessions` and `amux/turn_started` for UI attribution.
...
| `session/prompt` | ✅ | Stable v1 | Forwarded with id translation; turn serialization; ordinary concurrent prompts rejected with `-32001`; active-turn text-only `/steer ...` and `/queue ...` prompts are forwarded as Hermes sideband control requests without opening a new mux turn. |
