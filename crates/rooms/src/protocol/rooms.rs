//! `rooms/*` namespace frames — out-of-band metadata emitted by the
...
//! framing as needed). The shapes match `docs/design/rooms-namespace.md`.
//!
//! `roomsTurnId` is formatted `at-<u64>` with a monotonic per-session
//! counter; the prefix exists so a token shows its origin in logs.

use serde::Serialize;

const METHOD_PEER_JOINED: &str = "rooms/peer_joined";
const METHOD_PEER_LEFT: &str = "rooms/peer_left";
const METHOD_SESSION_CONTEXT: &str = "rooms/session_context";
const METHOD_TURN_STARTED: &str = "rooms/turn_started";
const METHOD_TURN_COMPLETE: &str = "rooms/turn_complete";
const METHOD_SESSION_BUSY: &str = "rooms/session_busy";
const METHOD_AGENT_REQUEST_OPENED: &str = "rooms/agent_request_opened";
const METHOD_AGENT_REQUEST_RESOLVED: &str = "rooms/agent_request_resolved";
const METHOD_TURN_CANCELLED: &str = "rooms/turn_cancelled";
const METHOD_CONTROL_SUBMITTED: &str = "rooms/control_submitted";
const METHOD_QUEUE_ITEM_ADDED: &str = "rooms/queue_item_added";
const METHOD_QUEUE_ITEM_SUBMITTED: &str = "rooms/queue_item_submitted";
const METHOD_QUEUE_ITEM_COMPLETED: &str = "rooms/queue_item_completed";
const METHOD_QUEUE_ITEM_REMOVED: &str = "rooms/queue_item_removed";
const METHOD_QUEUE_ITEM_ORPHANED: &str = "rooms/queue_item_orphaned";
const METHOD_REPLAY_STARTED: &str = "rooms/replay_started";
const METHOD_REPLAY_COMPLETE: &str = "rooms/replay_complete";
const METHOD_SEGMENT_STARTED: &str = "rooms/segment_started";
const METHOD_SEGMENT_ENDED: &str = "rooms/segment_ended";
/// Method name for the rooms extension that lets any attached peer steer the
...
/// as the next prompt.
pub const METHOD_STEER_ACTIVE_TURN: &str = "rooms/steer_active_turn";

/// Method name for the rooms extension that asks the mux to enqueue text for
/// the next turn while another turn is active.
pub const METHOD_QUEUE_PROMPT: &str = "rooms/queue_prompt";

/// Method name for the rooms extension that removes a queued prompt before it
...
pub const METHOD_UNQUEUE_PROMPT: &str = "rooms/unqueue_prompt";

/// Method name for the rooms extension that lets any attached peer cancel
...
pub const METHOD_CANCEL_ACTIVE_TURN: &str = "rooms/cancel_active_turn";

/// Resolved-by sentinel used in `rooms/agent_request_resolved` cleanup
...
pub struct RoomsTurnId(pub u64);
...
/// which a single canonical ACP `sessionId` is in force. A client-initiated
/// `session/load` or an observable upstream ACP `sessionId` change closes
/// one segment and opens the next. The first segment of a room opens once
...
/// Why a segment closed. Captured on `rooms/segment_ended` and exposed via
/// `/debug/sessions` so operators can distinguish explicit client-driven
/// loads from upstream session id changes observed in agent output.
...
pub enum EndReason {
    ...
    /// `sessionId` than the active segment's known value.
    AcpSessionIdChanged,
}
...
/// `rooms/cancel_active_turn`. Distinct from `rooms/turn_complete`, which
...
pub struct ControlSubmitted<'a> {
    ...
    pub rooms_turn_id: Option<RoomsTurnId>,
    pub text: &'a str,
}
...
pub fn turn_started(
    ...
    let id = rooms_turn_id.formatted();
    let supersedes = supersedes_turn_id.map(RoomsTurnId::formatted);
    encode(
        METHOD_TURN_STARTED,
        TurnStartedParams {
            room_id,
...
pub fn control_submitted(event: ControlSubmitted<'_>) -> Vec<u8> {
    let id = event.rooms_turn_id.map(RoomsTurnId::formatted);
    encode(
...
pub fn agent_request_opened(
    ...
    rooms_turn_id: Option<RoomsTurnId>,
) -> Vec<u8> {
    let id = rooms_turn_id.map(RoomsTurnId::formatted);
    encode(
...
/// room emits this alone (no `rooms/segment_ended` precedes it). Subsequent
/// segments emit `rooms/segment_ended` for the closing segment first, then
...
mod tests {
    ...
    #[test]
    fn turn_id_format() {
        assert_eq!(RoomsTurnId(42).formatted(), "at-42");
        assert_eq!(RoomsTurnId(0).formatted(), "at-0");
    }
    ...
    fn peer_joined_includes_optional_fields() {
        ...
        assert_eq!(v["method"], json!("rooms/peer_joined"));
    ...
    fn peer_left_shape() {
        ...
        assert_eq!(v["method"], json!("rooms/peer_left"));
        assert_eq!(v["params"]["roomId"], json!("work"));
    ...
    fn turn_started_carries_content() {
        ...
        assert_eq!(v["method"], json!("rooms/turn_started"));
        assert_eq!(v["params"]["roomsTurnId"], json!("at-7"));
        assert_eq!(v["params"]["peerId"], json!("phone-1"));
        assert_eq!(v["params"]["peerName"], json!("phone"));
        assert!(v["params"].get("role").is_none());
    ...
    fn turn_complete_shape() {
        ...
        let v = parse(&turn_complete("work", RoomsTurnId(7), &reason));
        assert_eq!(v["method"], json!("rooms/turn_complete"));
        assert_eq!(v["params"]["roomsTurnId"], json!("at-7"));
    ...
    fn session_busy_shape() {
        ...
        assert_eq!(v["method"], json!("rooms/session_busy"));
        assert_eq!(v["params"]["roomId"], json!("work"));
    ...
    fn agent_request_opened_shape() {
        ...
        assert_eq!(v["method"], json!("rooms/agent_request_opened"));
        assert!(v.get("id").is_none());
        assert_eq!(v["params"]["roomId"], json!("work"));
    ...
    fn agent_request_opened_omits_missing_optionals() {
        ...
        assert!(v["params"].get("roomsTurnId").is_none());
    }
    ...
    fn agent_request_resolved_with_result() {
        ...
        assert_eq!(v["method"], json!("rooms/agent_request_resolved"));
        assert_eq!(v["params"]["roomId"], json!("work"));
        assert_eq!(v["params"]["requestId"], req_id);
