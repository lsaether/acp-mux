...
//!   `_meta.rooms` fields to the current session/list entry so tests can
...
fn mock_client_tool_params(method: &str, session_id: &Value) -> Value {
    match method {
        "fs/read_text_file" => json!({
            "sessionId": session_id,
            "path": "/tmp/rooms-client-tool-read.txt",
        }),
        "fs/write_text_file" => json!({
            "sessionId": session_id,
            "path": "/tmp/rooms-client-tool-write.txt",
