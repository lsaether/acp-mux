...
//!
//! Subscriber attach query: `room`, `peer_id`, `peer_name?`, `role?`.
...
//! `room` is validated against `^[A-Za-z0-9_-]{1,128}$`. Missing required
//! fields or invalid room ids cause the upgraded socket to close with
//! application code 4400. `peer_id` already in use on a room closes with
//! 4409. Internal failures (no `--agent-cmd`, agent spawn failure) close
//! with code 1011.
//!
//! `?session=` is accepted as a deprecated alias for `?room=` during the
//! v0.2 transition. Each deprecated attach logs once at WARN; remove the
//! alias when no clients carry it any more.
...
use crate::room::registry::{ControlPlaneSessionListError, RegistryError, RoomRegistry};
use acp_mux::mux::MuxMsg;
...
pub struct AttachQuery {
    ...
    /// Deprecated alias for `room`. Accepted during the v0.2 transition;
    /// each request that uses it logs a one-shot WARN.
...
fn legacy_debug_room(mut room: serde_json::Value) -> serde_json::Value {
    let Some(obj) = room.as_object_mut() else {
        return room;
    };
    if let Some(mux_id) = obj.get("muxId").cloned() {
        obj.insert("roomId".to_string(), mux_id);
    }
    if let Some(prompt_in_flight) = obj.get("promptInFlight").cloned() {
        obj.insert("activeTurnMuxId".to_string(), prompt_in_flight);
    }
    let driving = obj
        .get("drivingSubscriber")
        .and_then(serde_json::Value::as_str)
        .map(str::to_string);
    if let Some(subscribers) = obj
        .get_mut("subscribers")
        .and_then(serde_json::Value::as_array_mut)
    {
        for subscriber in subscribers {
            let is_driving = subscriber
                .get("peerId")
                .and_then(serde_json::Value::as_str)
                .is_some_and(|peer_id| driving.as_deref() == Some(peer_id));
            if let Some(subscriber) = subscriber.as_object_mut() {
                subscriber.insert("isDriving".to_string(), serde_json::json!(is_driving));
            }
        }
    }
    room
...
async fn handle_attach(state: AppState, q: AttachQuery, mut socket: WebSocket) {
    ...
    let handle = match state.registry.attach(&room, subscriber).await {
        Ok(h) => h,
        Err(RegistryError::PeerIdInUse) => {
            tracing::warn!(%room, %peer_id, "peer_id collision");
            close_with(
                &mut socket,
                CLOSE_CODE_PEER_CONFLICT,
                "peer_id already attached to this room",
            )
            .await;
            return;
        }
        ...
        Err(RegistryError::ActorUnreachable) => {
            tracing::error!("room actor unreachable mid-attach");
            close_with(&mut socket, CLOSE_CODE_INTERNAL, "room unreachable").await;
            return;
        }
    };

    tracing::info!(%room, %peer_id, "subscriber attached");

    let (ws_sink, ws_stream) = socket.split();
    let in_room_tx = handle.tx.clone();
    let in_peer_id = peer_id.clone();
    let out_peer_id = peer_id.clone();
    let in_room = room.clone();
    let out_room = room.clone();

    tokio::select! {
        _ = ws_in_task(ws_stream, in_peer_id, in_room_tx, in_room) => {},
        _ = ws_out_task(ws_sink, outbound_rx, out_peer_id, out_room) => {},
    }

    // Whichever side ended first, make sure the room learns we're gone.
    // Send may fail if the room actor already exited; that's fine.
    let _ = handle
        .tx
        .send(MuxMsg::Detach {
        ...
        .await;
    tracing::debug!(%room, %peer_id, "ws handler exiting");
}

async fn ws_in_task(
    mut ws_stream: SplitStream<WebSocket>,
    peer_id: String,
    room_tx: mpsc::Sender<MuxMsg>,
    room: String,
) {
        ...
        match msg {
            Ok(Message::Text(t)) => {
                ...
                if room_tx
                    ...
                    .is_err()
                {
                    tracing::debug!(%room, %peer_id, "room actor gone; ws_in exiting");
                    return;
                }
            }
            Ok(Message::Binary(b)) => {
            ...
            Ok(Message::Close(_)) => {
                tracing::debug!(%room, %peer_id, "ws_in: client close");
                return;
            }
            Ok(Message::Ping(_)) | Ok(Message::Pong(_)) => {}
            Err(err) => {
                tracing::debug!(%room, %peer_id, error = %err, "ws recv error");
                return;
            }
        }
    }
...
async fn ws_out_task(
            ...
            OutMsg::Frame(bytes) => match Utf8Bytes::try_from(bytes) {
                Ok(text) => {
                    if ws_sink.send(Message::Text(text)).await.is_err() {
                        tracing::debug!(%room, %peer_id, "ws_out: peer dropped");
                        return;
                    }
                }
                Err(err) => {
                    tracing::warn!(%room, %peer_id, error = %err, "non-UTF8 agent stdout line; dropped");
                }
            },
            OutMsg::Close { code, reason } => {
                tracing::info!(%room, %peer_id, code, %reason, "ws_out: structured close");
                let _ = ws_sink
    ...
    tracing::debug!(%room, %peer_id, "ws_out: outbound channel closed");
...
fn validate(q: &AttachQuery) -> Result<ValidatedAttach, &'static str> {
    let room = match (q.room.as_deref(), q.session.as_deref()) {
        (Some(_), Some(_)) => return Err("specify either ?room or ?session, not both"),
        (Some(r), None) => r,
        (None, Some(legacy)) => {
            tracing::warn!(
                room = %legacy,
                "deprecated `?session=` query param; clients should switch to `?room=`",
            );
            legacy
        }
        (None, None) => return Err("missing ?room"),
    };
    if !is_valid_room_id(room) {
        return Err("invalid ?room (expect ^[A-Za-z0-9_-]{1,128}$)");
    }
...
mod tests {
    ...
    #[test]
    fn room_id_regex_accepts_canonical() {
        assert!(is_valid_room_id("abc"));
        assert!(is_valid_room_id("a_b-C-9"));
        assert!(is_valid_room_id(&"a".repeat(128)));
    }

    #[test]
    fn room_id_regex_rejects_edge_cases() {
        ...
        assert!(!is_valid_room_id("dotsbad."));
    }

    #[test]
    fn validate_requires_room_and_peer_id() {
        let q = AttachQuery {
            room: None,
            session: None,
    ...
    #[test]
    fn validate_accepts_deprecated_session_alias() {
        let q = AttachQuery {
            room: None,
            session: Some("legacy".into()),
            peer_id: Some("p1".into()),
            peer_name: None,
            role: None,
            replay: None,
        ...
        let v = validate(&q).unwrap();
        assert_eq!(v.room, "legacy");
    }

    #[test]
    fn validate_rejects_both_room_and_session() {
        let q = AttachQuery {
            room: Some("r".into()),
            session: Some("s".into()),
            peer_id: Some("p1".into()),
